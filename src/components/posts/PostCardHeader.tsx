    ...
    <div className={cn("flex justify-start items-center gap-3 w-full", className)}>
      <TooltipProvider>
        <UserProfileTooltip
          ...
          followOrUnfollow={mutate}
          isPending={isPending}
        >
          <div
            className="flex justify-start items-center gap-2 w-fit p-1 overflow-hidden"
            onMouseEnter={() => setOpenUserTooltip(true)}
            onMouseLeave={() => setOpenUserTooltip(false)}
          >
            <Link
              className={cn(isDeletingPost && "pointer-events-none")}
            ...
            <div className="flex flex-col justify-between items-start gap-0 w-fit overflow-hidden">
              <Link
                className={cn("w-full font-semibold text-neutral-900 truncate", isDeletingPost && "pointer-events-none")}
                to={`/profile/${postData.user.clerkId}`}
              >
